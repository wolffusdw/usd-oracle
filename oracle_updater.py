...
# Conecta à Mainnet TRON
client = Tron(network='mainnet')

def get_usdt_price():
    try:
        r = requests.get(ORACLE_URL)
        data = r.json()
        price = data['tether']['usd']
        return int(price * 1_000_000)
    except Exception as e:
        print("Erro na leitura do preço:", e)
        return None
...
def update_contract(rate):
    try:
        ...
        print("✅ Atualizado com sucesso! TX:", result['txid'])
    except Exception as e:
...
if __name__ == '__main__':
    rate = get_usdt_price()
    if rate:
        print("📡 Atualizando contrato com valor:", rate)
        update_contract(rate)
    else:
        print("❌ Falha ao obter a cotação do USDT")
